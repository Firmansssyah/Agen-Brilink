            ...
            <div className="mx-auto max-w-2xl">
                <div className="bg-[#2A282F] p-4 rounded-3xl">
                    <div className="flex justify-between items-center mb-4 px-2">
                        <h3 className="text-lg font-medium text-white">Jenis Transaksi</h3>
                        <button 
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            <thead className="border-b border-white/10">
                                <tr>
                                    <th className="p-3 text-xs font-medium uppercase text-[#958F99] tracking-wider">Nama Kategori</th>
                            ...
                            <tbody>
                                {categories.map((category, index) => (
                                     <tr key={index} className="border-b border-white/10 last:border-b-0 hover:bg-white/5 transition-colors duration-200">
                                        <td className="p-3 text-sm text-white">{category}</td>
                                        <td className="p-3 text-sm text-center">
                                            <div className="flex justify-center space-x-2">
                                                 <button onClick={() => handleOpenEditModal(category, index)} className="p-2 rounded-full text-slate-400 hover:bg-white/10 hover:text-white transition-colors duration-200"><EditIcon /></button>
                                                <button onClick={() => handleOpenDeleteModal(category, index)} className="p-2 rounded-full text-red-400 hover:bg-red-400/10 hover:text-red-300 transition-colors duration-200"><DeleteIcon /></button>
                                            </div>
                                        </td>
                                    </tr>
                                ))}
