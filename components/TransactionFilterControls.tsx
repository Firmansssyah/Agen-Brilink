...
import { SearchIcon, FilterIcon, CloseIcon } from './icons/Icons';
...
const TransactionFilterControls: React.FC<TransactionFilterControlsProps> = ({
    ...
    const inputClass = "w-full bg-slate-100 dark:bg-[#3C3A42] border border-transparent focus:border-indigo-400 focus:ring-1 focus:ring-indigo-400 rounded-lg p-2 text-sm text-slate-800 dark:text-white transition outline-none placeholder:text-slate-400 dark:placeholder:text-slate-500";
    ...
    return (
        <div className="px-2 pb-4 space-y-3">
            <div className="flex flex-col sm:flex-row items-center gap-3">
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto">
                    <button
                        ...
                        className={`flex flex-grow sm:flex-grow-0 justify-center items-center gap-2 px-4 py-2 text-sm font-semibold rounded-full transition-colors ${showFilters || hasActiveFilters ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-400/20 dark:text-indigo-200' : 'bg-slate-100 text-slate-600 dark:bg-[#3C3A42] dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600'}`}
                        aria-expanded={showFilters}
                    >
                        <FilterIcon className="h-4 w-4"/>
                        <span>Filter</span>
                         {hasActiveFilters && !showFilters && <span className="h-2 w-2 rounded-full bg-indigo-400"></span>}
                    </button>
            ...
            {showFilters && (
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 bg-slate-100/50 dark:bg-black/20 rounded-lg animate-fade-in">
                    <div>
