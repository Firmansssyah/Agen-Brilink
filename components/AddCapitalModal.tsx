...
const AddCapitalModal: React.FC<AddCapitalModalProps> = ({ isOpen, onClose, onSave, wallets }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            // Set default wallet to the first one in the list
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (amount <= 0) {
            setError('Jumlah modal harus lebih besar dari 0.');
            return;
        }
        if (!walletId) {
    ...
    return (
        <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="add-capital-modal-title"
        >
            <div
            ...
            >
                <div className="p-6 flex justify-between items-center border-b border-slate-200 dark:border-white/10">
                    <h2 id="add-capital-modal-title" className="text-xl font-medium text-slate-800 dark:text-white">Tambah Modal</h2>
                ...
                <form onSubmit={handleSubmit}>
                    <div className="p-6 space-y-5">
                        <p className="text-sm text-slate-500 dark:text-neutral-400">
                            Pilih dompet dan masukkan jumlah modal baru yang akan ditambahkan. Saldo dompet akan bertambah sesuai jumlah yang dimasukkan.
                        </p>
                         <div>
                            <label htmlFor="walletId" className="block text-sm font-medium text-slate-600 dark:text-neutral-300 mb-2">Dompet Tujuan</label>
